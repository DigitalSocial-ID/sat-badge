...
/// @title  DSID SAT renderer
/// @author Mohak Malhotra @mnm458


contract SatRenderer {
  constructor() {}

  /// @param _wallet The wallet of the recepient
  /// @param _grade The SAT grade of the recepient
  struct DsidStats {
  ...
  ///           DSID Finance SAT badge from the current template.
  /// @param _wallet The wallet of the recepient
  /// @param _grade The SAT grade of the recepient
  ...
  function renderPullRequest(
    ...
    DsidStats memory _dsidStats = DsidStats(
      _wallet,
      _grade
    );
    return string(abi.encodePacked(_buildPartOne(_dsidStats)));
  }
  ...
  /// @notice Build the SAT SVG in parts because otherwise it causes stack too deep errors
  function _buildPartOne(Contribution memory _dsidStats) internal pure returns (string memory) {
